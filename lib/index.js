...
/**
 * Dreamdata messages can be a maximum of 32kb.
 */
...
var Dreamdata = exports = module.exports = integration('Dreamdata.io')
  .option('apiKey', '')
  .option('apiHost', 'api.dreamdata.cloud/v1')
...
Dreamdata.prototype.initialize = function() {
  var self = this;

  if (this.options.retryQueue) {
    this._lsqueue = new Queue('dreamdataio', queueOptions, function(item, done) {
      // apply sentAt at flush time and reset on each retry
      // so the tracking-api doesn't interpret a time skew
      item.msg.sentAt = new Date();

      // send with 10s timeout
      Dreamdata.sendJsonWithTimeout(item.url, item.msg, item.headers, 10 * 1000, function(err, res) {
...
Dreamdata.prototype.loaded = function() {
  return true;
};
...
Dreamdata.prototype.onpage = function(page) {
...
Dreamdata.prototype.onidentify = function(identify) {
...
Dreamdata.prototype.ongroup = function(group) {
...
Dreamdata.prototype.ontrack = function(track) {
  var json = track.json();
...
Dreamdata.prototype.onalias = function(alias) {
  var json = alias.json();
...
Dreamdata.prototype.normalize = function(msg) {
...
Dreamdata.prototype.ampId = function(ctx) {
  var ampId = this.cookie('dreamdata_amp_id');
...
Dreamdata.prototype.enqueue = function(path, msg, fn) {
...
Dreamdata.prototype.cookie = function(name, val) {
  var store = Dreamdata.storage();
...
Dreamdata.prototype.referrerId = function(query, ctx) {
 ...
 * @api private
 */
Dreamdata.prototype.isCrossDomainAnalyticsEnabled = function() {
 ...
 */
Dreamdata.prototype.retrieveCrossDomainId = function(callback) {
  if (!this.isCrossDomainAnalyticsEnabled()) {
 ...
 */
Dreamdata.prototype.getCachedCrossDomainId = function() {
 ...
 * @api private
 */
Dreamdata.prototype.saveCrossDomainId = function(crossDomainId) {
  if (!this.options.saveCrossDomainIdInLocalStorage) {
 ...
 * @api private
 */
Dreamdata.prototype.deleteCrossDomainIdIfNeeded = function() {
...
function getJson(url, callback) {
  ...
  xhr.withCredentials = this.options.httpCookie || false;
